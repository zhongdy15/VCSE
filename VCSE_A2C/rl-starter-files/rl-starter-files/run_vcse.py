...
gpu_id_list = 2
task_set = {"GoToRedBallGrey","PutNext"}
task_type = "BabyAI"

for task in task_set:#{'SimpleCrossingS9N1'}:
    for seed in {1,2,3,4}:
        cmd_line = f"CUDA_VISIBLE_DEVICES={gpu_id_list} python3 -m scripts.train --algo a2c --env {task_type}-{task}-v0 --model {task+'_'+time_flag}/{task_type}-{task}-v0-vcse-{seed} " \
                   f"--save-interval 100 --frames 2000000 --use_entropy_reward --use_value_condition --seed {seed} --beta 0.005 --use_batch"
        print(cmd_line)
        os.system(cmd_line)
        time.sleep(5)
